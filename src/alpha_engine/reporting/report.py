...
from alpha_engine.evaluation.metrics import daily_ic
from alpha_engine.evaluation.performance import monthly_return_table
from alpha_engine.reporting import figures as F
from alpha_engine.utils.io import ensure_dir, read_json
...
def _write_markdown(res, figdir: Path, figs: dict[str, Path]) -> Path:
    ...
    A(f"python -m alpha_engine.cli run --config configs/{cfg.run.name}.yaml")
    A("```\n")
    t = res.timings or {}
    A(
...
def build_report_from_disk(cfg: Config) -> Path:
    ...
    from alpha_engine.backtest.engine import BacktestResult
    from alpha_engine.experiment import ExperimentResult
